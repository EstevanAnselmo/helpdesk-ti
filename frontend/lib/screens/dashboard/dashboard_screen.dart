...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Widget _buildDashboardState(Widget dashboard) {
    if (loading) {
      return const Center(
  ...
  Widget _dashboard() {
    return ListView(
      ...
      children: [
        Text(
          'Olá, ${widget.api.currentUser?.name ?? 'usuário'}',
          style: const TextStyle(
            fontSize: 28,
            fontWeight: FontWeight.bold,
          ),
        ...
        const Text(
          'Acompanhe o status do suporte de TI.',
        ),
        ...
        const SizedBox(height: 12),
        if (tickets.isEmpty)
          const Text('Nenhum chamado por aqui ainda.'),
        ...tickets.map(
          (ticket) => TicketTile(
            ticket: ticket,
